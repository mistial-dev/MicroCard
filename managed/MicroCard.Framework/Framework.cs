...
public interface IHost { int CommandLength { get; } void CopyCommandTo(byte[] destination,int destinationOffset,int sourceOffset,int length); void WriteResponse(byte[] source,int sourceOffset,int length); void Status(int value); int Get(int key); void Set(int key,int value); byte[] GetBytes(int key); void SetBytes(int key,byte[] value); void SetBytes(int key,byte[] value,int offset,int length); void DeleteBytes(int key); bool ContainsBytes(int key); void BeginTransaction(); void CommitTransaction(); void AbortTransaction(); int Random(); void FillRandom(byte[] destination,int offset,int length); int SecurityLevel { get; } byte[] GenerateKey(int slot,int algorithm); byte[] OpenKey(int slot); void DeleteKey(int slot); byte[] KeyOperation(int operation,byte[] token,byte[][] data); byte[] KeyOperation(int operation,byte[] token,byte[] data,int offset,int length); bool Ed25519Verify(byte[] publicKey,byte[] data,byte[] signature); bool P256Verify(byte[] publicKey,byte[] data,byte[] signature); void CreateCredential(int slot,byte[] pin,int pinOffset,int pinLength,int pinRetries,byte[] puk,int pukOffset,int pukLength,int pukRetries); bool VerifyCredential(int slot,byte[] candidate,int offset,int length); bool IsCredentialVerified(int slot); void ChangeCredential(int slot,byte[] newPin,int offset,int length); bool UnblockCredential(int slot,byte[] puk,int pukOffset,int pukLength,byte[] newPin,int newPinOffset,int newPinLength); int CredentialRetries(int slot,int kind); void Gpio(int resource,int value); }
...
public static class Cryptography {
 ...
 public static bool VerifyEd25519(byte[] publicKey,byte[] data,byte[] signature) => Host.Required.Ed25519Verify(publicKey,data,signature);
