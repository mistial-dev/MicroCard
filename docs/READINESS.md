...
- Complete JCVM transaction durability. Heap/static rollback, callback-end abort,
  ...
  and checkpoints still needs independent durability. P-256 generation now reserves
  rollback space for both key values and initialization flags before publication;
  scalar imports likewise update value and flags together. Catching a full-buffer
  error and committing cannot retain half a key update. Regression coverage also
  checks successful updates followed by abort and preservation of earlier undo records.
...
- Finish cross-link and host memory measurements for the Makerdiary JCVM image. A connected board
  ...
  revision and engine are unknown. The current JCVM dongle links at 198,192 text bytes,
  ...
  stages, then failed the board budget gate (56.337 seconds with two workers). All
  seven profiles cross-linked after the native key atomicity fix. After shared snapshot ownership changes, MC04
...
reuses outputs with bounded workers and isolated logs/state. The latest checkpoint took 56.337 seconds with two workers and incremental
