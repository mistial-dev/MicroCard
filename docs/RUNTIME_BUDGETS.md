...
[ASSEMBLY_BUDGETS.json](ASSEMBLY_BUDGETS.json) separately pins aggregate assembly and representative signed-package sizes. The five-assembly default ISD bundle is **10,663 MC04 bytes / 12,045 package bytes**. The four-assembly Credential deployment is **6,528 / 7,799 bytes** before per-device manifest-length differences.
