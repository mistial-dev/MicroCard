...
- SCP03 security levels 01, 03, 11, 13 and 33, which is every combination carrying a command MAC. Management requires that MAC and accepts any of them. Optional capabilities are cargo features, and both the advertised implementation option and the accepted levels are derived from the enabled set, so the card cannot offer protection it does not apply. A level outside that set is refused at EXTERNAL AUTHENTICATE with 6982.
