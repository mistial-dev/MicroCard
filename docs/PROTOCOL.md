...
The portable journal rotates through a fixed backend-reported count of two to eight slots. The nRF52840 profile uses three. An MJ03 record contains `MJ03`, a little-endian u64 committed-state generation, a little-endian u64 encryption-attempt number, a little-endian u32 ciphertext-plus-tag length, and AES-CCM ciphertext with a 16-byte tag. The 24-byte header is authenticated as associated data. The 13-byte nonce is `MCJN3 || attempt_le64`. The storage key is `AES-CMAC(SCP03-ENC, "MicroCard journal AEAD v1\\0" || SCP03-MAC)`, keeping it separate from direct protocol key use and binding it to both provisioned management keys. Fresh management keys are mandatory whenever persistent state is provisioned from empty storage so a nonce is never reused under the same key.

Before encryption, the backend durably consumes a fresh value from a separate append-only
nonce counter. Interrupted reservations consume their value even when encryption never
starts. A provider or flash failure cannot make a later attempt reuse that value, including
a retry after reboot. Recovery rejects a record whose attempt exceeds the reserved counter.
The record, tag, and slot markers require 43 bytes beyond the plaintext payload.

The final byte of the slot is the commit marker, programmed last. Before reusing the inactive slot, two bytes outside the record mark reclaim-started and reclaim-completed on the surviving slot. Recovery may ignore a corrupt peer only inside that explicit interval. After replacement completes, corruption remains fail-closed. Recovery chooses the highest valid authenticated generation, then re-verifies stored package signatures, domain targets, key binding, version digests and canonical unique registry AIDs. State is published in RAM only after all journal markers commit. MJ01/MJ02 journal state and incompatible snapshot versions are refused. There is no upgrade route.
...
Inventory is read-only. Complete enumeration should not be interleaved with management mutations. It is not a multi-command snapshot. Its optional empty-prefix filter reports diagnostic candidates but never authorizes or performs deletion. This command is not available on the original first-flash firmware.
