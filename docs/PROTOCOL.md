...
Only the basic logical channel is supported. `80 50 00 00 08 <host challenge> 00` initializes SCP03. Key version 0 or 1 is accepted. The card returns ten zero diversification bytes, key version 1, SCP identifier 03, the implementation option this build advertises, an eight-byte random challenge and eight-byte card cryptogram. `84 82 <level> 00 10 <host cryptogram><C-MAC>` authenticates. Levels 01, 03, 11, 13 and 33 are implemented, covering every combination that carries a command MAC. Management requires a command MAC and accepts any of them. A level requesting protection this build does not implement is refused with 6982.
