...
- AES-128 raw block encryption/decryption, AES-CBC encryption/decryption and AES-CCM authenticated encryption/decryption.
...
CBC and CCM providers also borrow their inputs and write into caller-owned output buffers.
...
The simulator, fuzz platform and explicit nRF52840 software-reference builds use RustCrypto. Default nRF52840 builds select the complete CC310 suite. Every provider entry point clears its complete caller-owned output before returning an error, including setup, bounds, driver, authentication and padding failures. `RuntimePlatform` applies the same rule to entropy even if a board entropy adapter fails after a partial write. The board's `cc310-sha256` feature links Nordic's pinned platform archive and overrides SHA-256 with its documented CC310 entry point. It uses fixed aligned DMA scratch, clears caller output on errors, and resets on a vendor abort. The `cc310-entropy` feature selects the documented CryptoCell TRNG path, requires complete output, clears partial output on failure, and rejects constant or repeated boot samples. The `cc310-cmac` and `cc310-hmac` features stream borrowed slices through the pinned TF-PSA multipart MAC wrapper and CC3XX driver. Their fixed 544-byte aligned operation state is wiped after every outcome, caller output changes only after an exact successful result, and the linked images contain no exact C `malloc`, `calloc`, `realloc` or `free` symbols. The `cc310-aes` feature borrows one AES-128 input block, clears it on failure and publishes its in-place result only after exact success. The `cc310-cbc` provider shares the core ISO 7816 padding implementation and performs CBC directly in the result allocation, wiping it on driver or padding failure. The `cc310-ccm` provider borrows every input and writes its exact result directly into caller-owned output through the one-shot AEAD wrapper. It maps tag rejection to authentication failure and wipes all output on every failure. The `cc310-p256` provider borrows fixed private-key, public-key and hash inputs and implements public-key derivation, deterministic ECDSA/SHA-256 sign/verify and ECDH through two exact pinned sdk-nrf CC3XX driver sources. It writes only into fixed caller-owned output, clears failure output, distinguishes invalid signatures or peer keys from provider failure, and retains raw private scalars only in the encrypted native key store. Boot adds exact RFC 6979 public-key/signature verification, changed-message rejection and a fixed ECDH answer to the symmetric known answers. Compile/link and memory evidence is recorded in `NRF52840_CC310_PLATFORM_SPIKE.json`. All eight features are enabled by the default `cc310` suite. The complete startup self-test has run successfully on the Makerdiary board. Forced provider failures, independent physical vectors, timing, memory, energy, and copy behavior remain unverified. Each further hardware override must pass the same known-answer, malformed-input, authentication-failure, and ownership tests.

Activation and authenticated recovery verify every package through the platform provider. Successful MC04 verification retains immutable manifest metadata shared by transaction candidates. A selected JCVM image is digest-verified when its slot is pinned; later APDUs borrow the same non-erasable mapping without hashing the image again. The JCVM loader still rebuilds its borrowed CAP view for each callback, which remains a measured cleanup target. Caches are absent from journal snapshots and are rebuilt through the provider on recovery. Provider errors fail activation or recovery.
...
check hashes split flash/RAM input; that startup check has run on the Makerdiary board,
while failure injection and sustained streaming tests remain open.
