...
| MCA0023 | More than 256 retained attributes | Only runtime attributes are retained and bounded | Generated schema ceiling and attribute-parent verifier enforce the signed table |
...
`scripts/profile_enforcement_audit.py` requires all 25 diagnostics in the analyzer, negative-build acceptance list, and this matrix. It also fixes the host and device enforcement anchors so a renamed or removed boundary fails the fast host gate.
