...
- `Nvm` rotates authenticated snapshots through three 64 KiB journal slots and keeps a separate 4 KiB append-only generation anchor behind the portable flash interface. The anchor clears one bit per generation for 32,768 commits. Recovery scans erased state in mapped flash, then copies only the fixed header, trailer and declared authenticated record. It never allocates a complete slot buffer. Runtime code has no anchor erase operation.
