...
- `Nvm` rotates authenticated snapshots through three 64 KiB journal slots and keeps separate 4 KiB append-only generation and nonce counters behind the portable flash interface. The counters clear one bit per committed generation and encryption attempt respectively, each bounded to 32,768 values. Recovery scans erased state in mapped flash, then copies only the fixed header, trailer and declared authenticated record. It never allocates a complete slot buffer. Runtime code has no anchor erase operation.
