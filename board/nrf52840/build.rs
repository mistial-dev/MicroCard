...
fn flash_layout(path: &str) -> String {
    ...
    for line in text.lines() {
        ...
        assert!(length > 0 && origin % 4096 == 0 && length % 4096 == 0,
            "{path}: {name} must use whole flash pages");
        assert!(regions.insert(name.to_owned(), (origin, end)).is_none(),
            "{path}: duplicate {name}");
    ...
    for (name, &(start, next)) in ordered {
        assert!(start >= end && next <= last,
            "{path}: {name} overlaps another region or protected firmware");
        end = next;
    }
    let image_bytes = regions["IMAGES"].1 - regions["IMAGES"].0;
    assert!(image_bytes % (16 * 1024) == 0 && (2..=64).contains(&(image_bytes / (16 * 1024))),
