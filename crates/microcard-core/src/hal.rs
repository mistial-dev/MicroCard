...
pub mod conformance {
    ...
    pub fn run_crypto<P: CryptoProvider>(provider: &mut P) -> Result<()> {
        ...
        provider.aes128_decrypt_block_in_place(&key, &mut block)?;
        require(block == plaintext)?;
        let iv = [
        ...
        let mut in_place = ccm;
        require(
            provider.aes_ccm_decrypt_in_place(&ccm_key, &nonce, &[], &mut in_place)
                == Err(Error::Authentication),
        )?;
        require(in_place == [0; 40])?;
        let mut truncated = [0xa5; 15];
        require(
