...
def main():
    ...
    for stage, features in STAGES.items():
        ...
        result.pop("ceilings")
        tree = subprocess.run(
            ["cargo", "tree", "--edges", "normal", "--prefix", "none", *arguments],
        ...
        result["features"] = ["development-debug", *features]
        results[stage] = result
        print(f"{stage}: {result['text_bytes']} text, {result['data_bytes']} data, {result['bss_bytes']} bss bytes")
    report = {"format": 1, "target": "thumbv7em-none-eabihf", "device_latency_measured": False, "stages": results}
    if args.output:
